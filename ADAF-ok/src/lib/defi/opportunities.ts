...
const CATEGORY_MATCHERS: Record<string, DefiCategory> = {
  lending: 'lending',
  stable: 'lending',
...
const PRIORITY_PROTOCOLS = new Set([
  'gearbox',
  'aave',
  'summerfi',
  'eigenlayer',
...
function detectCategory(project: string, pool: RawDefiLlamaPool): DefiCategory {
...
function detectRisk(pool: RawDefiLlamaPool): DefiRiskLevel {
...
const sortOpportunities = (a: DefiOpportunity, b: DefiOpportunity) => {
  const priorityA = PRIORITY_PROTOCOLS.has(normalize(a.protocol)) ? 1 : 0;
  const priorityB = PRIORITY_PROTOCOLS.has(normalize(b.protocol)) ? 1 : 0;
