...
import * as normStore from '@/lib/wsp/norm/store';
...
export function calculateWSPS(inputs: Inputs) {
  const factors = [
    {
      name: 'ETF_BTC',
      value: inputs.ETF_BTC_FLOW_NORM,
      weight: WSPS_WEIGHTS.ETF_BTC,
    },
    {
      name: 'ETF_ETH',
      value: inputs.ETF_ETH_FLOW_NORM,
      weight: WSPS_WEIGHTS.ETF_ETH,
    },
    { name: 'VIX', value: inputs.VIX_NORM, weight: WSPS_WEIGHTS.VIX },
    { name: 'DXY', value: inputs.DXY_NORM, weight: WSPS_WEIGHTS.DXY },
    {
      name: 'SPREAD_2s10s',
      value: inputs.SPREAD_2s10s_NORM,
      weight: WSPS_WEIGHTS.SPREAD_2s10s,
    },
    {
      name: 'SPX_NDX',
      value: inputs.SPX_NDX_MOM_NORM,
      weight: WSPS_WEIGHTS.SPX_NDX,
    },
  ];
  ...
  const score = Math.round(
    breakdown.reduce((acc, f) => acc + f.contribution, 0) * 100
  );
...
export async function smoothAndHysteresis(
  scoreRaw: number,
  alpha = 0.2,
  bandMargin = 3
) {
  ...
  const prevBand =
    (prevBandStr as 'green' | 'yellow' | 'red' | null) || memBand || 'red';
  const ema = Math.round(alpha * scoreRaw + (1 - alpha) * prev);

  // Determine desired band from current ema
  const desired: 'green' | 'yellow' | 'red' =
    ema >= WSPS_THRESHOLDS.green
      ? 'green'
      : ema >= WSPS_THRESHOLDS.yellow
        ? 'yellow'
        : 'red';
  // Apply hysteresis: only change if crosses threshold by bandMargin
  let band: 'green' | 'yellow' | 'red' = prevBand;
  if (prevBand === 'yellow') {
  ...
  return {
    score: ema,
    color: band,
    smoothing: { ema: true, hysteresis: { bandMargin } },
  };
}
...
export async function normalizeInputs(params: {
  ...
  // Read stats
  type PercentilePoint = { q?: Array<number | string> } | null;
  ...
  const [vixStats, dxyStats, btcPctl, ethPctl] = await Promise.all([
    normStore.readStats<WelfordState>(normStore.NORM_KEYS.vix),
    normStore.readStats<WelfordState>(normStore.NORM_KEYS.dxy),
    normStore.readPctl<PercentileSnapshot>(normStore.NORM_KEYS.etfBtc),
    normStore.readPctl<PercentileSnapshot>(normStore.NORM_KEYS.etfEth),
  ]);
  ...
  const vixStd =
    vixStats && vixStats.count >= 2
      ? Math.sqrt(vixStats.m2 / (vixStats.count - 1))
      : fb.vix.std;
  vixSource = vixStats && vixStats.count >= 2 ? 'redis' : 'fallback';
  const zClamp = (z: number) => clamp(z, -2.5, 2.5);
  const VIX_NORM =
    params.vix != null
      ? 1 - zClamp((params.vix - vixMean) / (vixStd || 1))
      : 0.5;
  ...
  const dxyStd =
    dxyStats && dxyStats.count >= 2
      ? Math.sqrt(dxyStats.m2 / (dxyStats.count - 1))
      : fb.dxy.std;
  dxySource = dxyStats && dxyStats.count >= 2 ? 'redis' : 'fallback';
  const DXY_NORM =
    params.dxy != null
      ? 1 - zClamp((params.dxy - dxyMean) / (dxyStd || 1))
      : 0.5;
  ...
  const extractPercentile = (
    snapshot: PercentileSnapshot | null | undefined
  ) => {
    const normalize = (value: number | string | undefined): number => {
      if (typeof value === 'number') return value;
      if (typeof value === 'string') return Number(value);
      return Number.NaN;
    };

    const p5Value = normalize(snapshot?.p5?.q?.[2]);
    const p95Value = normalize(snapshot?.p95?.q?.[2]);

    return {
      p5: Number.isFinite(p5Value) ? p5Value : fb.etf.p5,
      p95: Number.isFinite(p95Value) ? p95Value : fb.etf.p95,
    };
  };

  const btcPercentiles = extractPercentile(btcPctl);
  const ethPercentiles = extractPercentile(ethPctl);
  const btcP5 = btcPercentiles.p5;
  const btcP95 = btcPercentiles.p95;
  const ethP5 = ethPercentiles.p5;
  const ethP95 = ethPercentiles.p95;
  ...
  const SPREAD_2s10s_NORM =
    params.spread2s10s != null
      ? clamp((params.spread2s10s + 1) / 2, 0, 1)
      : 0.5;
  const SPX_NDX_MOM_NORM = params.spxNdXMom
    ? clamp((params.spxNdXMom.ndx + params.spxNdXMom.spx + 4) / 8, 0, 1)
    : 0.5;

  const source: 'redis' | 'fallback' =
    vixSource === 'redis' ||
    dxySource === 'redis' ||
    btcSource === 'redis' ||
    ethSource === 'redis'
