// Fortune 500-grade: Centralized ingestion handler factory and utilities (inlined for LAV-ADAF dashboard-backup)
...
export function createIngestHandler<T>({
	...
	return async function POST(request: NextRequest) {
			...
			if (Array.isArray(body)) {
				...
				for (const entry of body) {
					try {
						...
						const isNew = await redis.setnx(`dedupe:${hash}`, '1');
						if (isNew) {
		...
		try {
			...
			const extra = classify ? classify(item) : {};
			// Always return { success: true, signalId, fingerprint, ...extra } for single item
			return NextResponse.json({ success: true, signalId: hash, fingerprint: hash, ...extra }, { status: 201 });
	};
}
