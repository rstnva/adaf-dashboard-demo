...
export async function normalizeInputs(params: {
  ...
  const [vixStats, dxyStats, btcPctl, ethPctl] = await Promise.all([
    ...
    normStore.readPctl<{ p5: any; p95: any }>(normStore.NORM_KEYS.etfBtc),
    normStore.readPctl<{ p5: any; p95: any }>(normStore.NORM_KEYS.etfEth),
  ]);
  ...
  const btcP5 = btcPctl
    ? new Number((btcPctl as any).p5?.q?.[2] ?? fb.etf.p5).valueOf()
    : fb.etf.p5;
  const btcP95 = btcPctl
    ? new Number((btcPctl as any).p95?.q?.[2] ?? fb.etf.p95).valueOf()
    : fb.etf.p95;
  const ethP5 = ethPctl
    ? new Number((ethPctl as any).p5?.q?.[2] ?? fb.etf.p5).valueOf()
    : fb.etf.p5;
  const ethP95 = ethPctl
    ? new Number((ethPctl as any).p95?.q?.[2] ?? fb.etf.p95).valueOf()
    : fb.etf.p95;
