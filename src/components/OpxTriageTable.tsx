...
export default function OpxTriageTable({
  data,
  loading,
  onAction,
  onToast,
  ...
  const handleAction = async (
    ...
    try {
      ...
      onToast(
        `Failed to ${action}: ${e instanceof Error ? e.message : 'Unknown error'}`,
        true
      );
    } finally {
      setActioning(prev => {
          ...
          <tbody>
            {data.map(item => (
              <tr key={item.id} className="border-t hover:bg-muted/50">
                ...
                <td className="p-3">{Math.round(item.consensus * 100)}%</td>
                <td className="p-3">
                  <BlockingChips blocking={item.blocking} />
                </td>
                <td className="p-3">${item.var.toLocaleString()}</td>
                <td className="p-3">
                  ...
                  {item.status === 'proposed' ? (
                    <div className="flex gap-1">
                      <ActionModal
                        ...
                        id={item.id}
                        onConfirm={note =>
                          handleAction(item.id, 'approve', note)
                        }
                      >
                        <Button
                          size="sm"
                      ...
                      <ActionModal
                        ...
                        id={item.id}
                        onConfirm={note =>
                          handleAction(item.id, 'reject', note)
                        }
                      >
                        <Button
                          size="sm"
