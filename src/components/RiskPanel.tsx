    ...
    (async () => {
      try {
        const [v1, v7, d, l] = await Promise.all([
          fetch('/api/read/risk/var?window=1d', { cache: 'no-store' }).then(r =>
            r.json()
          ),
          fetch('/api/read/risk/var?window=7d', { cache: 'no-store' }).then(r =>
            r.json()
          ),
          fetch('/api/read/risk/dd?days=90', { cache: 'no-store' }).then(r =>
            r.json()
          ),
          fetch('/api/read/kpi/limits', { cache: 'no-store' }).then(r =>
            r.json()
          ),
        ]);
        if (cancelled) return;
...
export default function RiskPanel() {
  ...
  const ddArray = useMemo(() => (Array.isArray(dd) ? dd : []), [dd]);

  const ddMini = useMemo(
    () => ddArray.map(r => ({ date: r.date, value: r.ddPct })),
    [ddArray]
  );
  ...
  const limitsStructured = useMemo(() => {
    ...
    const limitsMap = Object.fromEntries(
      limits.limits.map((l: any) => [l.key, l.value])
    );
    const metricsMap = Object.fromEntries(
      (limits.metrics || []).map((m: any) => [m.key, m.value])
    );
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded border border-white/10 p-3">
              ...
              <div className="text-lg font-semibold">
                ${(var1d?.varUsd ?? 0).toLocaleString()}
              </div>
              <div className="text-xs opacity-80">
                {(var1d?.varPct ?? 0).toFixed(2)}%
              </div>
            </div>
            <div className="rounded border border-white/10 p-3">
              <div className="text-xs text-muted-foreground">VaR 7d</div>
              <div className="text-lg font-semibold">
                ${(var7d?.varUsd ?? 0).toLocaleString()}
              </div>
              <div className="text-xs opacity-80">
                {(var7d?.varPct ?? 0).toFixed(2)}%
              </div>
            </div>
            <div className="rounded border border-white/10 p-3">
              <div className="text-xs text-muted-foreground">DD actual</div>
              <div className="text-lg font-semibold">
                {ddCurrent ? `${ddCurrent.ddPct.toFixed(2)}%` : '—'}
              </div>
              <div className="text-xs opacity-80">
                ${ddCurrent ? ddCurrent.ddUsd.toLocaleString() : '—'}
              </div>
            </div>
            <div className="rounded border border-white/10 p-3">
              <div className="text-xs text-muted-foreground">DD máx 90d</div>
              <div className="text-lg font-semibold">
                {ddMax !== null ? `${ddMax.toFixed(2)}%` : '—'}
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {limitsStructured ? (
                <>
                  <Chip
                    name="Slippage"
                    value={limitsStructured.runtime.slippage_current}
                    limit={limitsStructured.limits.slippage}
                    tone="amber"
                  />
                  <Chip
                    name="LTV"
                    value={limitsStructured.runtime.ltv_current}
                    limit={limitsStructured.limits.ltv}
                    tone="emerald"
                  />
                  <Chip
                    name="HF"
                    value={limitsStructured.runtime.hf_current}
                    limit={limitsStructured.limits.hf}
                    tone="blue"
                    inverse
                  />
                  <Chip
                    name="RealYield"
                    value={limitsStructured.runtime.realyield_current}
                    limit={limitsStructured.limits.realyield}
                    tone="violet"
                  />
                </>
              ) : (
...
function Chip({
  ...
  const base = ok
    ? `bg-${tone}-50 text-${tone}-800 border-${tone}-200`
