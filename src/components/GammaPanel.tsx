...
export default function GammaPanel({ onError }: GammaPanelProps) {
  ...
  const [data, setData] = useState<GammaResponse>({
    tenor7: [],
    tenor14: [],
    tenor30: [],
  });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/read/derivs/gamma?asset=${asset}&tenors=7,14,30`,
        {
          cache: 'no-store',
        }
      );
  ...
  const chartData = useMemo(() => {
    const strikeMap = new Map<
      number,
      { strike: number; gamma7?: number; gamma14?: number; gamma30?: number }
    >();
  ...
  return (
    <div className="p-4 border rounded-lg bg-card">
      ...
      <div className="mt-3 text-xs text-muted-foreground">
        Gamma measures the rate of change in delta for option positions. Higher
        gamma indicates greater sensitivity to price moves.
      </div>
    </div>
  );
}
