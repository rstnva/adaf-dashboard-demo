...
export default function FundingPanel({ onError }: FundingPanelProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/read/derivs/funding?asset=${asset}&days=14`,
        {
          cache: 'no-store',
        }
      );
      ...
      const msg =
        e instanceof Error ? e.message : 'Failed to load funding data';
      onError?.(msg);
    } finally {
      setLoading(false);
    }
  ...
  const exchangeData = data.reduce(
    (acc, point) => {
      ...
      if (
        !acc[point.exchange].latest ||
        point.date > acc[point.exchange].latest!.date
      ) {
    ...
    <div className="p-4 border rounded-lg bg-card">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            value={asset}
            onValueChange={v => setAsset(v as 'BTC' | 'ETH')}
          >
              ...
              {exchanges.map(exchange => {
                ...
                return (
                  <tr key={exchange} className="border-b">
                    ...
                    <td className="p-2 text-muted-foreground">{latest.date}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="mt-3 text-xs text-muted-foreground">
        Funding rates show cost of holding perpetual positions. Negative rates
