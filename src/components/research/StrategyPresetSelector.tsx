...
import {
  ...
  type RiskLevel,
  type TimeHorizon,
} from '../../types/strategyPresets';

interface StrategyPresetSelectorProps {
  onSelectStrategy: (
    _strategy: StrategyTemplate,
    ...
    <div className="space-y-2">
      ...
      {parameter.type === 'selection' ? (
        ...
        <Input
          id={parameter.name}
          type={
            parameter.type === 'number' || parameter.type === 'percentage'
              ? 'number'
              : 'text'
          }
          value={value}
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center space-x-1">
            {Array.from({ length: preset.displayConfig.difficulty }).map(
              (_, i) => (
                <Star
                  key={i}
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="secondary">
              ...
              Target: {preset.successCriteria[0].target}
              {preset.successCriteria[0].unit}
            </Badge>
          </div>
...
function StrategyConfigDialog({
  ...
  React.useEffect(() => {
    if (template) {
      const defaultParams = template.preset.parameters.reduce(
        (acc, param) => ({
          ...acc,
  ...
  const handleApply = () => {
    ...
    const validation = validateStrategyParameters(
      template.preset.id,
      parameters
    );
    if (!validation.valid) {
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2">
            <span className="text-2xl">
              {template.preset.displayConfig.icon}
            </span>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        Max Drawdown:{' '}
                        {(
                          template.preset.riskMetrics.maxDrawdown * 100
                        ).toFixed(1)}
                        %
                      </div>
                      <div>
                        Volatility Target:{' '}
                        {(
                          template.preset.riskMetrics.volatilityTarget * 100
                        ).toFixed(1)}
                        %
                      </div>
                      <div>
                        Target Sharpe: {template.preset.riskMetrics.sharpeRatio}
                      </div>
                      <div>
                        Max Leverage: {template.preset.riskMetrics.maxLeverage}x
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div>
                        Risk Level: <Badge>{template.preset.riskLevel}</Badge>
                      </div>
                      <div>
                        Time Horizon:{' '}
                        <Badge>{template.preset.timeHorizon}</Badge>
                      </div>
                      <div>
                        Market Conditions:{' '}
                        {template.preset.marketConditions.join(', ')}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <TabsContent value="research" className="space-y-4">
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {template.preset.researchNotes.methodology}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    {template.preset.researchNotes.keyInsights.map(
                      (insight, i) => (
                ...
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    {template.preset.researchNotes.marketAssumptions.map(
                      (assumption, i) => (
                        <li key={i} className="flex items-start space-x-2">
                ...
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    {template.preset.researchNotes.limitations.map(
                      (limitation, i) => (
                        <li key={i} className="flex items-start space-x-2">
                ...
                {template.preset.researchNotes.references.length > 0 && (
                  <div>
                    ...
                    <ul className="text-sm text-muted-foreground space-y-1">
                      {template.preset.researchNotes.references.map(
                        (ref, i) => (
                          <li key={i} className="flex items-start space-x-2">
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <div className="text-sm text-muted-foreground">
            ...
            {template.preset.displayConfig.estimatedSetupTime}
          </div>
...
export function StrategyPresetSelector({
  ...
  const [configStrategy, setConfigStrategy] = useState<StrategyTemplate | null>(
    null
  );
  ...
  const filteredStrategies = useMemo(() => {
    return allStrategyTemplates.filter(template => {
      const matchesSearch =
        template.preset.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        template.preset.description
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        template.preset.tags.some(tag =>
          tag.toLowerCase().includes(searchTerm.toLowerCase())
        );

      const matchesCategory =
        selectedCategory === 'all' ||
        template.preset.category === selectedCategory;
      const matchesRisk =
        selectedRiskLevel === 'all' ||
        template.preset.riskLevel === selectedRiskLevel;
      const matchesHorizon =
        selectedTimeHorizon === 'all' ||
        template.preset.timeHorizon === selectedTimeHorizon;
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Professional strategy templates with pre-configured rules and
            parameters
          </p>
        </div>
        <Badge variant="secondary">
          {allStrategyTemplates.length} strategies
        </Badge>
      </div>
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <Select
          ...
          onValueChange={value =>
            setSelectedTimeHorizon(value as TimeHorizon | 'all')
          }
        >
          <SelectTrigger className="w-40">
            <SelectValue />
      ...
      {/* Results */}
      <div className="text-sm text-muted-foreground">
        Showing {filteredStrategies.length} of {allStrategyTemplates.length}{' '}
        strategies
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredStrategies.map(template => (
          <StrategyCard
            ...
            onSelect={() =>
              setSelectedStrategy(
                selectedStrategy === template.preset.id
                  ? null
                  : template.preset.id
              )
            }
