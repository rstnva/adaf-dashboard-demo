...
export function ResearchPanel() {
  ...
  useEffect(() => {
    if (
      ...
      config.rules?.some(r => r.expr)
    ) {
      const { valid, errors } = ResearchApi.validateConfig(
  ...
  const handleRuleChange = (
    index: number,
    field: keyof (typeof config.rules)[0],
    value: string | number
  ) => {
    setConfig(prev => ({
      ...prev,
      rules:
        prev.rules?.map((rule, i) =>
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (response.success && response.status === 'done') {
        ...
        const detailResponse = await ResearchApi.getBacktest(
          selectedBacktest.id
        );
        ...
        throw new Error(
          response.errorMessage || response.error || 'Backtest execution failed'
        );
      }
    } catch (error) {
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      if (response.success && response.opportunity) {
        ...
        // Navigate to OPX page (now implemented with mock data)
        router.push('/opx');
      } else {
    ...
    try {
      ...
      const snapshot = createSnapshot(
        selectedBacktest.name,
        {
          ...
          tradingParams: {
            maxPositionSize:
              selectedBacktest.config.sizing?.notionalPctNAV || 0.1,
   ...
   */
  const promotionCheck = selectedBacktest
    ? ResearchApi.canPromote(selectedBacktest)
    : { canPromote: false };
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handlePresetSelection(
                        key as keyof typeof STRATEGY_PRESETS
                      )
                    }
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={value =>
                    setConfig(prev => ({
                      ...prev,
                      benchmark: value as 'BTC' | 'ETH' | 'NAV',
                    }))
                  }
                >
                  <SelectTrigger>
                ...
                <Card key={index}>
                  <CardContent className="p-4 space-y-3">
                    ...
                    <Textarea
                      value={rule.expr}
                      onChange={e =>
                        handleRuleChange(index, 'expr', e.target.value)
                      }
                    ...
                    <div>
                      ...
                      <Slider
                        value={[rule.weight || 1]}
                        onValueChange={([value]) =>
                          handleRuleChange(index, 'weight', value)
                        }
                        min={0.1}
                        max={5}
                        step={0.1}
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label className="text-sm">
                  Tamaño de Posición:{' '}
                  {(config.sizing?.notionalPctNAV || 0.1) * 100}%
                </Label>
                <Slider
              ...
              <div>
                <Label className="text-sm">
                  Fees: {config.feesBps || 5} bps
                </Label>
                <Slider
                  value={[config.feesBps || 5]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({ ...prev, feesBps: value }))
                  }
                  min={0}
                  max={200}
                  step={1}
              ...
              <div>
                <Label className="text-sm">
                  Slippage: {config.slippageBps || 3} bps
                </Label>
                <Slider
                  value={[config.slippageBps || 3]}
                  onValueChange={([value]) =>
                    setConfig(prev => ({ ...prev, slippageBps: value }))
                  }
                  min={0}
                  max={100}
                  step={1}
              ...
              {canRun && (
                <Button
                  ...
                  disabled={
                    isRunning ||
                    !selectedBacktest ||
                    selectedBacktest.status !== 'queued'
                  }
                  variant="secondary"
                  className="w-full"
                >
            ...
            {selectedBacktest ? (
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      ...
                      •{selectedBacktest.config.agents.join(', ')} • benchmark{' '}
                      {selectedBacktest.config.benchmark}
                    </CardDescription>
                  </CardHeader>
                  ...
                  {selectedBacktest.results && (
                    <CardContent>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                        <div className="text-center p-3 bg-muted rounded">
                          <div className="text-2xl font-bold">
                            {ResearchApi.formatKpi(
                              selectedBacktest.results.kpis.pnlPct,
                              'percentage'
                            )}
                          </div>
                          <div className="text-xs text-muted-foreground">
                        ...
                        <div className="text-center p-3 bg-muted rounded">
                          <div className="text-2xl font-bold">
                            {ResearchApi.formatKpi(
                              selectedBacktest.results.kpis.sharpe,
                              'ratio'
                            )}
                          </div>
                          <div className="text-xs text-muted-foreground">
                        ...
                        <div className="text-center p-3 bg-muted rounded">
                          <div className="text-2xl font-bold">
                            {ResearchApi.formatKpi(
                              selectedBacktest.results.kpis.maxDDPct,
                              'percentage'
                            )}
                          </div>
                          <div className="text-xs text-muted-foreground">
                        ...
                        <div className="text-center p-3 bg-muted rounded">
                          <div className="text-2xl font-bold">
                            {ResearchApi.formatKpi(
                              selectedBacktest.results.kpis.hitRate,
                              'percentage'
                            )}
                          </div>
                          <div className="text-xs text-muted-foreground">
                        ...
                        <div className="text-center p-3 bg-muted rounded">
                          <div className="text-2xl font-bold">
                            {ResearchApi.formatKpi(
                              selectedBacktest.results.kpis.vsBenchmarkPct,
                              'percentage'
                            )}
                          </div>
                          <div className="text-xs text-muted-foreground">
                      ...
                      <div className="space-y-6">
                        ...
                        <MonthlyPnLChart
                          data={selectedBacktest.results.monthlyPnL}
                        />
                      </div>

                      {/* Notes/Warnings */}
                      {selectedBacktest.results.notes &&
                        selectedBacktest.results.notes.length > 0 && (
                          <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded">
                            <h4 className="font-medium text-amber-800 mb-2">
                              Notas y Advertencias
                            </h4>
                            <ul className="text-sm text-amber-700 space-y-1">
                              {selectedBacktest.results.notes.map(
                                (note, index) => (
                                  <li
                                    key={index}
              ...
              <Card>
                <CardContent className="p-12 text-center text-muted-foreground">
                  ...
                  <p>
                    Selecciona un backtest de la lista para ver resultados y
                    gráficos
                  </p>
                </CardContent>
              </Card>
