...
export function LessonCard({ lesson, onLessonUpdate }: LessonCardProps) {
  ...
  return (
    <Card
      className={`hover:shadow-lg transition-shadow duration-200 ${statusConfig[lesson.progress.status].bgColor} border`}
    >
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-1">
            <StatusIcon
              className={`h-5 w-5 ${statusConfig[lesson.progress.status].color}`}
            />
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-gray-700 line-clamp-3">{lesson.summary}</p>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium">
              {Math.round(lesson.progress.completionPercentage)}%
            </span>
          </div>
        ...
        {lesson.progress.quizScore !== null && (
          <div className="flex items-center justify-between text-sm">
            ...
            <span
              className={`font-medium ${lesson.progress.quizScore >= 80 ? 'text-green-600' : 'text-orange-600'}`}
            >
