...
export class RateLimiter {
  ...
  async checkLimit(
    ...
    try {
      ...
      let bucket =
        this.memoryStore.get(key) || (await this.redisStore.get(key));
...
export const RATE_LIMIT_CONFIGS: Record<string, number> = {
  ...
  // Feature Store API limits
  '/api/feature-store/catalog': parseInt(
    process.env.RATE_FEATURE_CATALOG_PER_MIN || '300'
  ),
  '/api/feature-store/*/latest': parseInt(
    process.env.RATE_FEATURE_LATEST_PER_MIN || '300'
  ),
  '/api/feature-store/query': parseInt(
    process.env.RATE_FEATURE_QUERY_PER_MIN || '180'
  ),
  '/api/feature-store/publish': parseInt(
    process.env.RATE_FEATURE_PUBLISH_PER_MIN || '60'
  ),
