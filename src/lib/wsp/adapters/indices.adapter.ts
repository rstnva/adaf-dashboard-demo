...
export async function getIndices(): Promise<{ data: Indices; stale: boolean }> {
  const ENDPOINT =
    process.env.WSP_INDICES_API_URL || process.env.INDICES_API_URL || '';
  const API_KEY =
    process.env.WSP_INDICES_API_KEY || process.env.INDICES_API_KEY || '';
  ...
  try {
    const { body, status, stale } = await fetchWithCacheETag(
      ADAPTER,
      ENDPOINT,
      cacheKey,
      CACHE_TTL,
    ...
    if (!stale && status === 200 && typeof parsed.vix === 'number') {
      const prev = await readStats<{ mean: number; m2: number; count: number }>(
        NORM_KEYS.vix
      );
