...
export async function getCatalysts({
  ...
  window: string;
}): Promise<{ data: Catalyst[]; stale: boolean }> {
  const ENDPOINT =
    process.env.WSP_CALENDAR_API_URL || process.env.CALENDAR_API_URL || '';
  const API_KEY =
    process.env.WSP_CALENDAR_API_KEY || process.env.CALENDAR_API_KEY || '';
