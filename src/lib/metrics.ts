...
// Simulation metrics
export const cBlockspaceRequests = new client.Counter({
  name: 'adaf_blockspace_request_total',
  help: 'Blockspace simulations executed',
  labelNames: ['decision', 'mev_protection'] as const,
});

export const hBlockspaceLatency = new client.Histogram({
  name: 'adaf_blockspace_latency_seconds',
  help: 'Latency of blockspace simulations',
  labelNames: ['mev_protection'] as const,
  buckets: [0.02, 0.05, 0.1, 0.25, 0.5, 1],
});

export const hVaultsSimLatency = new client.Histogram({
  name: 'adaf_vaults_sim_latency_seconds',
  help: 'Latency of LAV vault simulations',
  labelNames: ['liquidity_band'] as const,
  buckets: [0.05, 0.1, 0.2, 0.4, 0.8, 1.5],
});

export const cAlphaFactorySignals = new client.Counter({
  name: 'adaf_alpha_factory_signal_total',
  help: 'Signals generated by the Alpha Factory simulation',
  labelNames: ['strategy'] as const,
});

export const cEventAlphaAlerts = new client.Counter({
  name: 'adaf_event_alpha_alert_total',
  help: 'Event alpha alerts classified by severity',
  labelNames: ['severity'] as const,
});

export const gLiquidityBackstopReserveRatio = new client.Gauge({
  name: 'adaf_liquidity_backstop_reserve_ratio',
  help: 'Reserve multiplier recommended by the liquidity backstop planner',
  labelNames: ['desk'] as const,
});
...
export function recordBlockspaceSimulation(
  decision: 'accepted' | 'rejected',
  mevProtection: boolean,
  durationMs?: number
) {
  try {
    const mevLabel = mevProtection ? 'enabled' : 'disabled';
    cBlockspaceRequests.labels({ decision, mev_protection: mevLabel }).inc();
    if (
      typeof durationMs === 'number' &&
      Number.isFinite(durationMs) &&
      durationMs >= 0
    ) {
      hBlockspaceLatency.labels({ mev_protection: mevLabel }).observe(durationMs / 1000);
    }
  } catch {
    /* no-op */
  }
...
export function recordVaultsSimulationLatency(
  ...
  try {
    if (
      ...
      hVaultsSimLatency
        .labels({ liquidity_band: liquidityBand || 'unknown' })
...
export function recordAlphaFactorySignals(strategies: string[]) {
  try {
    if (!Array.isArray(strategies) || strategies.length === 0) {
      cAlphaFactorySignals.labels({ strategy: 'unknown' }).inc();
      return;
    }
    strategies.forEach(strategy => {
      cAlphaFactorySignals.labels({ strategy }).inc();
    });
  } catch {
    /* no-op */
  }
...
export function recordEventAlphaAlerts(severities: string[]) {
  try {
    if (!Array.isArray(severities) || severities.length === 0) {
      cEventAlphaAlerts.labels({ severity: 'unknown' }).inc();
      return;
    }
    severities.forEach(severity => {
      cEventAlphaAlerts.labels({ severity }).inc();
    });
  } catch {
    /* no-op */
  }
...
export function setLiquidityBackstopReserveRatio(
  desk: string,
  reserveMultiplier: number
) {
  try {
    if (typeof reserveMultiplier === 'number' && Number.isFinite(reserveMultiplier)) {
      gLiquidityBackstopReserveRatio
        .labels({ desk: desk || 'unknown' })
        .set(Math.max(0, reserveMultiplier));
    }
  } catch {
    /* no-op */
  }
...
registry.registerMetric(cBlockspaceRequests);
registry.registerMetric(hBlockspaceLatency);
registry.registerMetric(hVaultsSimLatency);
registry.registerMetric(cAlphaFactorySignals);
registry.registerMetric(cEventAlphaAlerts);
registry.registerMetric(gLiquidityBackstopReserveRatio);
