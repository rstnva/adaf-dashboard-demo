...
export const cApiErrors = new client.Counter({
  name: 'adaf_api_errors_total',
  help: 'API requests resulting in error status (>=400)',
  labelNames: ['route', 'method', 'status'] as const,
});

export const hApiRequestDuration = new client.Histogram({
...
export function incApiRequest(
  ...
  try {
    ...
    cApiRequests
      .labels({ route, method: normalizedMethod, status: statusLabel })
      .inc();
    ...
    if (
      ...
      hApiRequestDuration
        .labels({ route, method: normalizedMethod })
...
export function incOpxAction(
  action: 'APPROVE' | 'REJECT',
  status: 'ok' | 'noop' | 'error'
) {
  try {
    cOpxActions.labels({ action, status }).inc();
  } catch {
    /* no-op */
  }
}
...
export function setOpxBacklog(
  status: 'proposed' | 'approved' | 'rejected',
  value: number
) {
  try {
    gOpxBacklog.labels({ status }).set(value);
  } catch {
    /* no-op */
  }
}
...
export function incDerivsFundingAlert(asset: 'BTC' | 'ETH', exchange: string) {
  try {
    cDerivsFundingAlerts.labels({ asset, exchange }).inc();
  } catch {
    /* no-op */
  }
}

export function setDerivsFundingNegHours(
  asset: 'BTC' | 'ETH',
  exchange: string,
  hours: number
) {
  try {
    gDerivsFundingNegHours.labels({ asset, exchange }).set(hours);
  } catch {
    /* no-op */
  }
}
...
export function incDqpIncident(
  kind: string,
  source: string,
  agentCode: string
) {
  try {
    cDqpIncidents.labels({ kind, source, agentCode }).inc();
  } catch {
    /* no-op */
  }
}

export function setDqpSourcesStatus(
  status: 'ok' | 'warn' | 'fail',
  count: number
) {
  try {
    gDqpSourcesStatus.labels({ status }).set(count);
  } catch {
    /* no-op */
  }
}

export function setDqpLastFreshness(
  source: string,
  agentCode: string,
  type: string,
  minutes: number
  ...
  try {
    gDqpLastFreshnessMinutes.labels({ source, agentCode, type }).set(minutes);
  } catch {
    /* no-op */
  }
}
...
export function incLineageEvent(
  stage: 'ingest' | 'transform' | 'aggregate' | 'export',
  entity: 'signal' | 'metric' | 'report',
  source: string
) {
  try {
    cLineageEvents.labels({ stage, entity, source }).inc();
  } catch {
    /* no-op */
  }
}
...
export function observeLineageSearchDuration(seconds: number) {
  try {
    hLineageSearchDuration.observe(seconds);
  } catch {
    /* no-op */
  }
}
...
export function incLineageView(entity: 'signal' | 'metric' | 'report') {
  try {
    cLineageViews.labels({ entity }).inc();
  } catch {
    /* no-op */
  }
}
...
registry.registerMetric(cApiErrors);
registry.registerMetric(hApiRequestDuration);
