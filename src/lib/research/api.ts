...
export class ResearchApi {
  ...
  static async runBacktest(
    ...
    return apiRequest<RunBacktestResponse>(
      '/backtest/run',
      {
        method: 'POST',
        body: JSON.stringify(request),
      ...
      120000
    ); // 2 minutes timeout for backtest execution
  }
   ...
   */
  static async promoteBacktest(
    request: PromoteBacktestRequest
  ): Promise<PromoteBacktestResponse> {
   ...
   */
  static validateConfig(config: BacktestConfig): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    // Fees validation
    if (
      config.feesBps !== undefined &&
      (config.feesBps < 0 || config.feesBps > 1000)
    ) {
    ...
    if (
      ...
      (config.slippageBps < 0 || config.slippageBps > 1000)
    ) {
   ...
   */
  static canPromote(backtest: Backtest): {
    canPromote: boolean;
    reason?: string;
  } {
  ...
  static formatKpi(
    value: number,
    type: 'percentage' | 'ratio' | 'count' | 'currency'
  ): string {
    switch (type) {
      case 'percentage':
...
export const AGENT_OPTIONS = [
  ...
  },
  {
    code: 'OF-1',
    name: 'ETF Flows',
    description: 'Spot ETF inflow/outflow tracking',
  },
  {
    code: 'OC-1',
    name: 'On-Chain Analytics',
    description: 'DeFi TVL and on-chain metrics',
  },
  {
    code: 'DV-1',
    name: 'Derivatives Volume',
    description: 'Futures and options analysis',
  },
  {
    code: 'DV-2',
    name: 'Derivatives Structure',
    description: 'Gamma and options flow',
  },
  {
    code: 'MX-1',
    name: 'Market Microstructure',
    description: 'Order book and execution analytics',
  },
];
...
export const BENCHMARK_OPTIONS = [
  ...
  { code: 'ETH', name: 'Ethereum', description: 'Ethereum price performance' },
  {
    code: 'NAV',
    name: 'Cash/NAV',
    description: 'Flat benchmark (no market exposure)',
  },
] as const;
