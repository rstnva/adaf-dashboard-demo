...
type RedisLike = {
  ...
  set: (_key: string, _value: string) => Promise<'OK' | null>;
  setex: (_key: string, _ttl: number, _value: string) => Promise<'OK' | null>;
  setnx: (_key: string, _value: string) => Promise<boolean>;
  del: (..._keys: string[]) => Promise<number>;
  ttl: (_key: string) => Promise<number>;
  mget: (..._keys: string[]) => Promise<(string | null)[]>;
  ...
  pipeline: () => {
    setex: (_key: string, _ttl: number, _value: string) => any;
    set: (_key: string, _value: string) => any;
    sadd: (_key: string, _value: string) => any;
    expire: (_key: string, _ttl: number) => any;
    exec: () => Promise<any>;
  };
  publish?: (_channel: string, _message: string) => Promise<number>;
  psubscribe?: (_pattern: string) => Promise<number>;
  on?: (_event: string, _cb: (..._args: any[]) => void) => void;
  ping?: () => Promise<string>;
...
const isMock = () =>
  process.env.MOCK_MODE === '1' || process.env.NODE_ENV === 'test';
...
const memClient: RedisLike = {
  ...
  async setex(key, ttl, value) {
    mem.set(key, { v: value, exp: now() + ttl * 1000 });
    return 'OK';
  },
  async del(...keys) {
    let c = 0;
  ...
  async ttl(key) {
    const e = mem.get(key);
    if (!e) return -2;
    return Math.max(0, Math.floor((e.exp - now()) / 1000));
  },
  async mget(...keys) {
    return keys.map(k => {
      const e = mem.get(k);
  ...
  async sadd(_key, _member) {
    return 1;
  },
  async expire(key, ttl) {
    const e = mem.get(key);
    if (!e) return 0;
    mem.set(key, { v: e.v, exp: now() + ttl * 1000 });
    return 1;
  },
  async keys(pattern) {
    const re = new RegExp('^' + pattern.replace('*', '.*') + '$');
    return Array.from(mem.keys()).filter(k => re.test(k));
  },
  ...
  pipeline() {
    ...
    return {
      setex: (k, t, v) => {
        ops.push(() => {
          mem.set(k, { v, exp: now() + t * 1000 });
        });
        return this;
      },
      set: (k, v) => {
        ops.push(() => {
          mem.set(k, { v, exp: now() + 24 * 3600 * 1000 });
        });
        return this;
      },
      sadd: (_k, _v) => {
      ...
      expire: (k, t) => {
        ops.push(() => {
          const e = mem.get(k);
          if (e) mem.set(k, { v: e.v, exp: now() + t * 1000 });
        });
        return this;
      },
      async exec() {
...
export function getSafeRedis(opts?: RedisOptions): RedisLike {
  ...
  try {
    ...
    const { default: IORedis } = require('ioredis') as {
      default: new (_options?: RedisOptions | string) => Redis;
    };
...
export function getMemoryRedis(): RedisLike {
  return memClient;
}
