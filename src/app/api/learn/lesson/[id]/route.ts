...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    `;
    const quiz =
      Array.isArray(quizResult) && quizResult.length > 0 ? quizResult[0] : null;
    ...
    `;
    const checklist =
      Array.isArray(checklistResult) && checklistResult.length > 0
        ? checklistResult[0]
        : null;
    ...
    if (userId !== 'anonymous') {
      ...
      userProgress =
        Array.isArray(progressResult) && progressResult.length > 0
          ? progressResult[0]
          : {
              status: 'not_started',
    ...
    if (lesson.prerequisites && lesson.prerequisites.length > 0) {
      ...
      relatedLessons.prerequisites = Array.isArray(prereqResult)
        ? prereqResult
        : [];
    }
    ...
    const nextStepsResult = await db.$queryRaw`
      ...
      LIMIT 5
    `;
    relatedLessons.nextSteps = Array.isArray(nextStepsResult)
      ? nextStepsResult
      : [];
    ...
    console.info('Lesson detail fetch duration (ms)', durationMs);
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const existingLesson =
      await db.$queryRaw`SELECT id FROM lessons WHERE id = ${lessonId}`;
    ...
    const durationMs = Date.now() - startTime;
    console.info('Lesson update duration (ms)', durationMs);
