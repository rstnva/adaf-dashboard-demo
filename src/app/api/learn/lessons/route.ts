...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { kind, difficulty, tag, search, enabled, limit, offset } =
      lessonsQuerySchema.parse(searchParams);
    ...
    if (search) {
      conditions.push(
        `(title ILIKE $${paramIndex++} OR summary ILIKE $${paramIndex} OR array_to_string(tags, ' ') ILIKE $${paramIndex})`
      );
    ...
    const lessonsResult = (await db.$queryRawUnsafe(
      lessonsQuery,
      ...params
    )) as any[];
    ...
    const countResult = (await db.$queryRawUnsafe(
      countQuery,
      ...params.slice(0, -2)
    )) as any[]; // Remove limit/offset from count query
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existingLesson =
      (await db.$queryRaw`SELECT id FROM lessons WHERE code = ${validatedData.code}`) as any[];
