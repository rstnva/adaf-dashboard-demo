...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const scorePercentage = Math.round(
      (correctAnswers / quizItems.length) * 100
    );
    ...
    console.info('Quiz submission duration (ms)', durationMs);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (userId) {
      ...
      const attemptsResult = await db.$queryRawUnsafe(attemptsQuery, [
        userId,
        lessonId,
      ]);
    ...
    const durationMs = Date.now() - startTime;
    console.info('Quiz retrieval duration (ms)', durationMs);
