...
class ExerciseHandler {
  ...
  async runBacktest(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/api/research/backtest/run`,
        {
          method: 'POST',
          headers: {
  ...
  async promoteOpx(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/api/research/backtest/promote`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  async generateReport(
    ...
    try {
      const response = await fetch(
        `${this.baseUrl}/api/generate/report/onepager`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    try {
      ...
      if (environment !== 'staging') {
        return {
          ...
          result: {
            error:
              'CSP enforcement changes only allowed in staging environment',
          },
        };
      }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const requiredRoleLevel =
      roleOrder[exercise.required_role as keyof typeof roleOrder] || 1;
    ...
    const environment = process.env.NEXT_PUBLIC_ENV || 'production';
    if (
      exercise.environment_restriction &&
      exercise.environment_restriction !== environment
    ) {
      return NextResponse.json(
        {
    ...
    console.info('Academy exercise execution duration (ms)', durationMs);
  ...
  switch (method) {
    case 'api':
      ...
      try {
        ...
        return {
          passed: false,
          details: { error: 'API call failed', status: response.status },
        };
      } catch (error) {
    ...
    case 'table':
      ...
      try {
        const result = (await db.$queryRawUnsafe(
          `SELECT COUNT(*) as count FROM ${params.table} WHERE ${params.condition}`
        )) as any[];
    ...
    case 'fileHash':
      ...
      if (executionData && executionData.fileHash) {
        const hashMatches = params.expected_hash_pattern
          ? new RegExp(params.expected_hash_pattern).test(
              executionData.fileHash
            )
          : true;
      ...
      return {
        passed: false,
        details: { error: 'No file hash in execution result' },
      };

    default:
      return {
        passed: false,
        details: { error: 'Unknown verification method' },
      };
  }
}
