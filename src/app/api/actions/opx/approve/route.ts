const sanitizeNote = (value: string | undefined | null) => {
  if (!value) return '';
...
async function approveHandler(req: NextRequest) {
  ...
  try {
    ...
    const note = sanitizeNote(body.note).slice(0, 500);
    const now = new Date();
    const [row] = await prisma.$queryRaw<Array<{ status: string }>>(
      Prisma.sql`SELECT status FROM opportunities WHERE id=${id} LIMIT 1`
    );
    if (!row) {
    ...
    if ((row.status || '').toLowerCase() === 'approved') {
      await prisma.$executeRaw(
        Prisma.sql`INSERT INTO change_logs(actor, entity, entityId, field, old, new, at) VALUES (${actor}, 'Opportunity', ${id}, 'APPROVE', ${JSON.stringify({ note })}, ${JSON.stringify({ id, status: 'approved' })}, ${now})`
      );
      const res = NextResponse.json({ ok: true, id, status: 'approved' });
      incOpxAction('APPROVE', 'noop');
      incApiRequest('/api/actions/opx/approve', 'POST', res.status);
      return res;
    }
    await prisma.$executeRaw(
      Prisma.sql`UPDATE opportunities SET status='approved', "updatedAt"=${now} WHERE id=${id}`
    );
    ...
    try {
      ...
      >(
        Prisma.sql`SELECT status, COUNT(*)::int AS c FROM opportunities GROUP BY status`
      );
      ...
      console.warn(
        'Failed to refresh OP-X backlog metrics after approval',
        error
      );
    }
    await prisma.$executeRaw(
