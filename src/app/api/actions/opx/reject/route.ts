  ...
  try {
    ...
    if ((row.status || '').toLowerCase() === 'rejected') {
      await prisma.$executeRaw(
        Prisma.sql`INSERT INTO change_logs(actor, entity, entityId, field, old, new, at) VALUES (${actor}, 'Opportunity', ${id}, 'REJECT', ${JSON.stringify({ note })}, ${JSON.stringify({ id, status: 'rejected' })}, ${now})`
      );
      const res = NextResponse.json({ ok: true, id, status: 'rejected' });
      incOpxAction('REJECT', 'noop');
      incApiRequest('/api/actions/opx/reject', 'POST', res.status);
      return res;
    }
    await prisma.$executeRaw(
      Prisma.sql`UPDATE opportunities SET status='rejected', "updatedAt"=${now} WHERE id=${id}`
    );
    ...
    try {
      ...
      console.warn(
        'Failed to refresh OP-X backlog metrics after rejection',
        error
      );
    }
    await prisma.$executeRaw(
