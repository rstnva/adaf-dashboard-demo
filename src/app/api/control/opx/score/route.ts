...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rows = await prisma.$queryRaw<
      Array<{ id: string; metadata: OpxMeta | null; signalId: string | null }>
    >(Prisma.sql`
    ...
    for (const r of rows) {
      ...
      try {
        ...
        console.warn(
          'Failed to derive consensus for opportunity',
          { id: r.id },
          error
        );
      }
      ...
      try {
        ...
        console.warn(
          'Failed to derive base severity for opportunity',
          { id: r.id, signalId: r.signalId },
          error
        );
      }
      ...
      const score = Math.min(
        Math.max(base + adj - guardrailPenalty - varPenalty, 0),
        100
      );
