...
import {
  ...
  smoothAndHysteresis,
  normalizeInputs,
} from '@/components/dashboard/wsp/utils/scoring';
...
export async function GET(_req: NextRequest) {
  ...
  try {
    if (!tokenBucket('route:wsp:wsps', 5, 10))
    ...
    const mom = indices?.data?.dChange
      ? { spx: indices.data.dChange.spx, ndx: indices.data.dChange.ndx }
      : null;
    const { inputs, normalization } = await normalizeInputs({
      vix,
      dxy,
      etfBtcFlow: btcFlow,
      etfEthFlow: null,
    ...
    return new Response(
      JSON.stringify({
        ...
        color: smoothed.color,
        factors: base.breakdown,
        ts: Date.now(),
