...
export function useSpotlight(options?: UseSpotlightOptions) {
  ...
  const allCommands = useMemo(() => {
    ...
    if (!query.trim()) {
      const recentStaticCommands = staticCommands
      ...
      const otherCommands = staticCommands.filter(
    ...
    return [...staticCommands, ...entityResults]
      .sort((a, b) => {
        const aScore =
          (('score' in a ? a.score : undefined) as number) ||
          (a.priority || 50) / 100;
        const bScore =
          (('score' in b ? b.score : undefined) as number) ||
          (b.priority || 50) / 100;
  ...
  const executeCommand = useCallback(
    async (command: SpotlightCommand) => {
      postUiEvent('Spotlight', 'Execute', {
        cmdId: command.id,
      ...
      try {
        switch (command.action) {
          ...
          case 'generate-report': {
            ...
            await fetch(`/api/generate/report/${reportType}`, {
              method: 'POST',
            });
            break;
          }
