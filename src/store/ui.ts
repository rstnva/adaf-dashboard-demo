...
export interface UIState {
  ...
  setRange: (_range: UIState['range']) => void;
  setCurrency: (_currency: UIState['currency']) => void;
  setTimezone: (_timezone: UIState['timezone']) => void;
  setAsOf: (_asOf: Date | 'now') => void;
  toggleSidebar: () => void;
  toggleSearch: () => void;
...
export const useAssetAwareParams = () => {
  ...
  return {
    ...
    getQueryKey: (baseKey: string) => [
      baseKey,
      { assets: selectedAssets, range, currency },
    ],
  };
};
