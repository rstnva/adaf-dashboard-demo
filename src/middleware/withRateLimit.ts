...
export async function withRateLimit(
  request: NextRequest,
  handler: (_req: NextRequest) => Promise<NextResponse> | NextResponse
  ...
  try {
    ...
    if (!result.allowed) {
      ...
      if (rateLimitBlockCounter) {
        rateLimitBlockCounter
          .labels({ route: routePattern, ip: clientIP })
          .inc();
      }

      console.warn(
        `Rate limit exceeded for IP ${clientIP} on route ${routePattern}`
      );
...
export function rateLimit(requestsPerMinute?: number) {
  return function <
    T extends (..._args: any[]) => Promise<NextResponse> | NextResponse,
  >(handler: T) {
