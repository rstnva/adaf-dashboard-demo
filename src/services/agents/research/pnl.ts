...
export class PnLSimulator {
  ...
  private readonly costs: TradingCosts;
  private readonly sizing: PositionSizing;
  ...
  processDay(
    ...
    const trades = this.executePositionChange(
      date,
      targetPosition,
      underlyingPrice
    );
  ...
  private executePositionChange(
    ...
    if (targetPosition !== 0) {
      if (
        Math.sign(targetPosition) !== Math.sign(oldPosition) ||
        oldPosition === 0
      ) {
        ...
        this.entryPrice =
          (this.entryPrice * Math.abs(oldPosition) + price * addedQuantity) /
          totalQuantity;
      }
    }
  ...
  smoothPosition(
    ...
    return (
      smoothingFactor * targetPosition + (1 - smoothingFactor) * this.position
    );
  }
   ...
   */
  static validateCosts(costs: TradingCosts): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
   ...
   */
  static validateSizing(sizing: PositionSizing): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      sizing.minPosition !== undefined &&
      sizing.maxPosition !== undefined &&
