...
export class BenchmarkService {
  ...
  private static cache: Map<
    string,
    { data: BenchmarkData[]; timestamp: number }
  > = new Map();
   ...
   */
  private static async getBTCReturns(
    fromDate: string,
    toDate: string
  ): Promise<BenchmarkData[]> {
   ...
   */
  private static async getETHReturns(
    fromDate: string,
    toDate: string
  ): Promise<BenchmarkData[]> {
    // TODO: Replace with actual database query
   ...
   */
  private static async getNAVReturns(
    fromDate: string,
    toDate: string
  ): Promise<BenchmarkData[]> {
   ...
   */
  private static async queryMetricsTable(
    metricKey: string,
    fromDate: string,
    toDate: string
  ): Promise<BenchmarkData[]> {
    ...
    console.warn(
      `⚠️ Using mock data for ${metricKey} - implement database query`
    );
   ...
   */
  private static generateFlatReturns(
    fromDate: string,
    toDate: string
  ): BenchmarkData[] {
   ...
   */
  private static generateSyntheticReturns(
    fromDate: string,
    toDate: string,
    benchmark: Benchmark
  ): BenchmarkData[] {
   ...
   */
  private static generateMockBenchmarkData(
    metricKey: string,
    fromDate: string,
    toDate: string,
    days: number
   ...
   */
  private static generateRealisticReturn(
    metricKey: string,
    dayIndex: number
  ): number {
   ...
   */
  static validateBenchmarkData(data: BenchmarkData[]): {
    valid: boolean;
    warnings: string[];
  } {
    ...
    for (let i = 1; i < dates.length; i++) {
      ...
      if (dates[i].getTime() !== expectedDate.getTime()) {
        warnings.push(
          `Data gap detected around ${dates[i].toISOString().split('T')[0]}`
        );
      }
    }
    ...
    if (maxReturn > 0.5 || minReturn < -0.5) {
      warnings.push(
        `Extreme returns detected: ${(minReturn * 100).toFixed(1)}% to ${(maxReturn * 100).toFixed(1)}%`
      );
    }
    ...
    if (maxPrice > 0 && maxPrice > minPrice * 1_000) {
      warnings.push(
        'Price range spans over three orders of magnitude; verify data normalization'
      );
    }
